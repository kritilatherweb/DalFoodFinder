...
const Contact= () => {
  ...
  const onSubmit = async (event) => {
    ...
    if(data.success){
        setResult("");
        toast.success("Form Submitted Successfulyy")
        event.target.reset();
    }
    else
    {
  ...
  return (
    <motion.div initial={{opacity: 0, x:-200}} transition={{duration: 1.5}}
        ...
        <p className='text-center text-gray-500 mb-12 max-w-80 mx-auto'>ets Us build future together</p>
    ...
    <div className='my-6 text-left'>Message
        <textarea className='w-full border border-gray-300 rounded py-3 px-4 mt-2 h-48 resze-none' name="Message" placeholder='Message' required></textarea>
